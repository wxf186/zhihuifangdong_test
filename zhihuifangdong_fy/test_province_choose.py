...
class TestDistrict:
    ...
    def test_12_province_choose(self):
        """省份列表获取"""
        print("\n📋 测试省份列表获取...")
        response = api_request("GET", "/core/app/house/provinceChoose")
        print(f"响应: {json.dumps(response, ensure_ascii=False, indent=2)[:300]}")
        validate_response(response)
        print(f"✅ 省份列表获取成功")
        record_result("test_12_province_choose", "/core/app/house/provinceChoose", response, "PASSED", group=TEST_GROUP)
