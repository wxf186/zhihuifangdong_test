...
class TestBindCard:
    ...
    def test_27_get_bind_card_fail_pop(self):
        ...
        print("\n💳 测试绑卡失败弹窗...")
        response = api_request("GET", "/core/app/bindCard/getBindCardFailPop")
        print(f"响应: {json.dumps(response, ensure_ascii=False, indent=2)}")
        validate_response(response, allow_success_false=True)
        print("✅ 绑卡失败弹窗查询完成")
        record_result("test_27_get_bind_card_fail_pop", "/core/app/bindCard/getBindCardFailPop", response, "PASSED", group=TEST_GROUP)
