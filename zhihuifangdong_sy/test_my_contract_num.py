...
class TestContract:
    ...
    def test_15_my_contract_num(self):
        """我的合同数量"""
        print("\n📑 测试我的合同数量...")
        response = api_request("GET", "/core/app/contractExt/myContractNum")
        print(f"响应: {json.dumps(response, ensure_ascii=False, indent=2)}")
        validate_response(response)
        print("✅ 合同数量获取成功")
        record_result("test_15_my_contract_num", "/core/app/contractExt/myContractNum", response, "PASSED", group=TEST_GROUP)
