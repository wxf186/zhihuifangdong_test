...
class TestMeter:
    ...
    def test_31_water_meter_list(self):
        """水表设备列表"""
        print("\n💧 测试水表设备列表...")
        response = api_request("GET", "/core/app/house/waterIndexList",
                               params={"current": 1, "size": 10})
        print(f"响应: {json.dumps(response, ensure_ascii=False, indent=2)}")
        validate_response(response)
        print("✅ 水表设备列表查询完成")
        record_result("test_31_water_meter_list", "/core/app/house/waterIndexList", response, "PASSED", group=TEST_GROUP)
