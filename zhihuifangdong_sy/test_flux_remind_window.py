...
class TestMeter:
    ...
    def test_25_flux_remind_window(self):
        ...
        _start = time.time()
        time.sleep(0.5)
        response = api_request("GET", "/netty/web/meter/fluxRemindWindow",
                               params={"userId": USER_ID},
                               timeout=60)
        print(f"响应: {json.dumps(response, ensure_ascii=False, indent=2)}")
        try:
            validate_response(response)
            record_result("test_25_flux_remind_window", "/netty/web/meter/fluxRemindWindow", response, "PASSED", group=TEST_GROUP)
        except AssertionError:
            record_result("test_25_flux_remind_window", "/netty/web/meter/fluxRemindWindow", response, "FAILED", group=TEST_GROUP)
            raise
        finally:
            if TEST_RESULTS:
