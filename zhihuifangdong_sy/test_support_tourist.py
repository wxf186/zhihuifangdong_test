...
class TestHousePromotion:
    ...
    def test_29_support_tourist(self):
        """房源推广支持"""
        print("\n🏠 测试房源推广支持...")
        response = api_request("GET", "/findhouse/app/houseScore/supportTourist")
        print(f"响应: {json.dumps(response, ensure_ascii=False, indent=2)}")
        validate_response(response)
        print("✅ 房源推广支持查询完成")
        record_result("test_29_support_tourist", "/findhouse/app/houseScore/supportTourist", response, "PASSED", group=TEST_GROUP)
