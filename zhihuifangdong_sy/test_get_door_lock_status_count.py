...
class TestDevice:
    ...
    def test_23_get_door_lock_status_count(self):
        """门锁状态统计"""
        print("\n🔒 测试门锁状态统计...")
        response = api_request("GET", "/netty/app/doorlock/getDoorLockStatusCount")
        print(f"响应: {json.dumps(response, ensure_ascii=False, indent=2)}")
        validate_response(response)
        print("✅ 门锁状态统计获取成功")
        record_result("test_23_get_door_lock_status_count", "/netty/app/doorlock/getDoorLockStatusCount", response, "PASSED", group=TEST_GROUP)
