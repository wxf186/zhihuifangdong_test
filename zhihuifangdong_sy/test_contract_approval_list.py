...
class TestContract:
    ...
    def test_17_contract_approval_list(self):
        """合同审批列表"""
        print("\n📑 测试合同审批列表...")
        response = api_request("POST", "/core/contractApproval/list",
                               json_data={"approvalStatus": "INIT", "current": 1, "size": 9999})
        print(f"响应: {json.dumps(response, ensure_ascii=False, indent=2)}")
        validate_response(response)
        print("✅ 合同审批列表获取成功")
        record_result("test_17_contract_approval_list", "/core/contractApproval/list", response, "PASSED", group=TEST_GROUP)
