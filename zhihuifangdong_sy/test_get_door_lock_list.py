...
class TestDevice:
    ...
    def test_24_get_door_lock_list(self):
        """门锁设备列表"""
        print("\n🔒 测试门锁设备列表...")
        response = api_request("GET", "/core/app/house/lockIndexList",
                               params={"current": 1, "size": 10})
        print(f"响应: {json.dumps(response, ensure_ascii=False, indent=2)}")
        validate_response(response)
        print("✅ 门锁设备列表查询完成")
        record_result("test_24_get_door_lock_list", "/core/app/house/lockIndexList", response, "PASSED", group=TEST_GROUP)
