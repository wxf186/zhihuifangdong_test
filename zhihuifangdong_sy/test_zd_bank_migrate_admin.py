...
class TestBindCard:
    ...
    def test_28_zd_bank_migrate_admin(self):
        """银行迁移状态"""
        print("\n💳 测试银行迁移状态...")
        response = api_request("GET", "/core/app/bindCard/zdBankMigrateAdmin")
        print(f"响应: {json.dumps(response, ensure_ascii=False, indent=2)}")
        validate_response(response)
        print("✅ 银行迁移状态查询完成")
        record_result("test_28_zd_bank_migrate_admin", "/core/app/bindCard/zdBankMigrateAdmin", response, "PASSED", group=TEST_GROUP)
