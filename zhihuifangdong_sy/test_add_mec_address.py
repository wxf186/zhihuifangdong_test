...
class TestFeign:
    ...
    def test_30_add_mec_address(self):
        ...
        print("\n📡 测试MAC地址上报...")
        time.sleep(0.3)
        response = api_request("POST", "/core/feign/userExtend/addMecAddress",
                               data={"macAddress": "BFE2048DB90664D3EBF6CB14306202BA"})
        print(f"响应: {json.dumps(response, ensure_ascii=False, indent=2)}")
        validate_response(response)
        print("✅ MAC地址上报完成")
        record_result("test_30_add_mec_address", "/core/feign/userExtend/addMecAddress", response, "PASSED", group=TEST_GROUP)
