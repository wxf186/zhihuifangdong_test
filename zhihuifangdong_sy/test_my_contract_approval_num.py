...
class TestContract:
    ...
    def test_16_my_contract_approval_num(self):
        """待审批合同数量"""
        print("\n📑 测试待审批合同数量...")
        response = api_request("GET", "/core/app/contractExt/myContractApprovalNum")
        print(f"响应: {json.dumps(response, ensure_ascii=False, indent=2)}")
        validate_response(response)
        print("✅ 待审批合同数量获取成功")
        record_result("test_16_my_contract_approval_num", "/core/app/contractExt/myContractApprovalNum", response, "PASSED", group=TEST_GROUP)
