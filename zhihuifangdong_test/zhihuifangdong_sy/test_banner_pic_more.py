...
# -*- coding: utf-8 -*-
"""
智慧房东 APP - /core/app/activity/bannerPicMore 接口测试
"""
...
class TestActivity:
    """/core/app/activity/bannerPicMore"""

    def test_18_banner_pic_more(self):
        """Banner图片"""
        print("\n🎨 测试Banner图片...")
        response = base_tester.api_request("GET", "/core/app/activity/bannerPicMore")
        print(f"响应: {base_tester.json.dumps(response, ensure_ascii=False, indent=2)}")
        base_tester.validate_response(response)
        print("✅ Banner图片获取成功")
        base_tester.record_result("test_18_banner_pic_more", "/core/app/activity/bannerPicMore", response, "PASSED", group=TEST_GROUP)
...
if __name__ == "__main__":
    ...
    method = getattr(instance, "test_18_banner_pic_more")
    METHOD_RECORDED = False
    try:
        method()
        print("\n✅ " + "test_18_banner_pic_more" + " 执行通过")
