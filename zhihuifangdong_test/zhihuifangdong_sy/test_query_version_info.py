...
# -*- coding: utf-8 -*-
"""
智慧房东 APP - /core/app/activity/queryVersionInfo 接口测试
"""
...
class TestActivity:
    """/core/app/activity/queryVersionInfo"""

    def test_22_query_version_info(self):
        """版本信息查询"""
        print("\n📱 测试版本信息查询...")
        response = base_tester.api_request("GET", "/core/app/activity/queryVersionInfo")
        print(f"响应: {base_tester.json.dumps(response, ensure_ascii=False, indent=2)}")
        base_tester.validate_response(response)
        print("✅ 版本信息查询完成")
        base_tester.record_result("test_22_query_version_info", "/core/app/activity/queryVersionInfo", response, "PASSED", group=TEST_GROUP)
...
if __name__ == "__main__":
    ...
    method = getattr(instance, "test_22_query_version_info")
    METHOD_RECORDED = False
    try:
        method()
        print("\n✅ " + "test_22_query_version_info" + " 执行通过")
        ...
        base_tester.record_result(
            "test_22_query_version_info",
            api_path_val,
            exc_response if exc_response else {"success": False, "message": str(e)},
            "FAILED"
        , group=TEST_GROUP)
        print("\n❌ " + "test_22_query_version_info" + " 执行失败: " + str(e))
