...
class TestUserInfo:
    ...
    def test_04_self_menu(self):
        """用户菜单列表"""
        print("\n📋 测试用户菜单列表...")
        response = base_tester.api_request("GET", "/core/app/user/selfMenu")
        print(f"响应: {base_tester.json.dumps(response, ensure_ascii=False, indent=2)[:500]}")
        base_tester.validate_response(response)
        print("✅ 用户菜单列表获取成功")
        base_tester.record_result("test_04_self_menu", "/core/app/user/selfMenu", response, "PASSED", group=TEST_GROUP)
