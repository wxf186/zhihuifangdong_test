...
# -*- coding: utf-8 -*-
"""
智慧房东 APP - /core/app/user/certificateExpirationRemind 接口测试
"""
...
class TestUserInfo:
    """/core/app/user/certificateExpirationRemind"""

    def test_09_certificate_expiration_remind(self):
        """证书过期提醒"""
        print("\n📋 测试证书过期提醒...")
        response = base_tester.api_request("GET", "/core/app/user/certificateExpirationRemind")
        print(f"响应: {base_tester.json.dumps(response, ensure_ascii=False, indent=2)[:500]}")
        base_tester.validate_response(response)
        print("✅ 证书过期提醒查询完成")
        base_tester.record_result("test_09_certificate_expiration_remind", "/core/app/user/certificateExpirationRemind", response, "PASSED", group=TEST_GROUP)
...
if __name__ == "__main__":
    ...
    method = getattr(instance, "test_09_certificate_expiration_remind")
    METHOD_RECORDED = False
    try:
        method()
        print("\n✅ " + "test_09_certificate_expiration_remind" + " 执行通过")
        ...
        base_tester.record_result(
            "test_09_certificate_expiration_remind",
            api_path_val,
            exc_response if exc_response else {"success": False, "message": str(e)},
            "FAILED"
        , group=TEST_GROUP)
        print("\n❌ " + "test_09_certificate_expiration_remind" + " 执行失败: " + str(e))
