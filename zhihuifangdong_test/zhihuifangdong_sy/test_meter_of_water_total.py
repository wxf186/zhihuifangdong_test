...
class TestHouse:
    ...
    def test_14_meter_of_water_total(self):
        """水表数据汇总"""
        print("\n💧 测试水表数据汇总...")
        response = base_tester.api_request("GET", "/core/app/house/meterOfwaterTotal")
        print(f"响应: {base_tester.json.dumps(response, ensure_ascii=False, indent=2)}")
        base_tester.validate_response(response)
        print("✅ 水表数据汇总获取成功")
        base_tester.record_result("test_14_meter_of_water_total", "/core/app/house/meterOfwaterTotal", response, "PASSED", group=TEST_GROUP)
