...
class TestActivity:
    ...
    def test_20_click_pic_more(self):
        """更多点击图片"""
        print("\n🎨 测试更多点击图片...")
        response = base_tester.api_request("GET", "/core/app/activity/clickPicMore")
        print(f"响应: {base_tester.json.dumps(response, ensure_ascii=False, indent=2)}")
        base_tester.validate_response(response)
        print("✅ 更多点击图片获取成功")
        base_tester.record_result("test_20_click_pic_more", "/core/app/activity/clickPicMore", response, "PASSED", group=TEST_GROUP)
