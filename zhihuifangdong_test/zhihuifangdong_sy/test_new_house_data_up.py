...
# -*- coding: utf-8 -*-
"""
智慧房东 APP - /core/app/house/newHouseDataUp 接口测试
"""
...
class TestHouse:
    """/core/app/house/newHouseDataUp"""

    def test_12_new_house_data_up(self):
        """房源数据概览"""
        print("\n🏠 测试房源数据概览...")
        response = base_tester.api_request("GET", "/core/app/house/newHouseDataUp")
        print(f"响应: {base_tester.json.dumps(response, ensure_ascii=False, indent=2)}")
        base_tester.validate_response(response)
        assert "data" in response
        print("✅ 房源数据概览获取成功")
        base_tester.record_result("test_12_new_house_data_up", "/core/app/house/newHouseDataUp", response, "PASSED", group=TEST_GROUP)
...
if __name__ == "__main__":
    ...
    instance = TestHouse()
    method = getattr(instance, "test_12_new_house_data_up")
    METHOD_RECORDED = False
    try:
        method()
        print("\n✅ " + "test_12_new_house_data_up" + " 执行通过")
        ...
        base_tester.record_result(
            "test_12_new_house_data_up",
            api_path_val,
            exc_response if exc_response else {"success": False, "message": str(e)},
            "FAILED"
        , group=TEST_GROUP)
        print("\n❌ " + "test_12_new_house_data_up" + " 执行失败: " + str(e))
