...
# -*- coding: utf-8 -*-
"""
智慧房东 APP - /core/app/user/commissionBear 接口测试
"""
...
class TestUserInfo:
    """/core/app/user/commissionBear"""

    def test_08_commission_bear(self):
        """佣金承担查询"""
        print("\n📋 测试佣金承担查询...")
        response = base_tester.api_request("GET", "/core/app/user/commissionBear")
        print(f"响应: {base_tester.json.dumps(response, ensure_ascii=False, indent=2)}")
        base_tester.validate_response(response)
        print("✅ 佣金承担查询完成")
        base_tester.record_result("test_08_commission_bear", "/core/app/user/commissionBear", response, "PASSED", group=TEST_GROUP)
...
if __name__ == "__main__":
    ...
    method = getattr(instance, "test_08_commission_bear")
    METHOD_RECORDED = False
    try:
        method()
        print("\n✅ " + "test_08_commission_bear" + " 执行通过")
