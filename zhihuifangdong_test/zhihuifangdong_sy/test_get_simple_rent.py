...
# -*- coding: utf-8 -*-
"""
智慧房东 APP - /core/app/landlordSet/getSimpleRent 接口测试
"""
...
class TestHouse:
    """/core/app/landlordSet/getSimpleRent"""

    def test_11_get_simple_rent(self):
        """简化租金设置查询"""
        print("\n🏠 测试简化租金设置查询...")
        response = base_tester.api_request("GET", "/core/app/landlordSet/getSimpleRent")
        print(f"响应: {base_tester.json.dumps(response, ensure_ascii=False, indent=2)}")
        base_tester.validate_response(response)
        print("✅ 简化租金设置查询完成")
        base_tester.record_result("test_11_get_simple_rent", "/core/app/landlordSet/getSimpleRent", response, "PASSED", group=TEST_GROUP)
...
if __name__ == "__main__":
    ...
    instance = TestHouse()
    method = getattr(instance, "test_11_get_simple_rent")
    METHOD_RECORDED = False
    try:
        method()
        print("\n✅ " + "test_11_get_simple_rent" + " 执行通过")
