...
def _record_duration(ms: int):
    if base_tester.TEST_RESULTS:
        base_tester.TEST_RESULTS[-1]["duration"] = ms
...
if __name__ == "__main__":
    ...
    print(f"   域名: {api_config.get_base_url()}")
    print(f"   账号: {current_user['username']}")
    print("="*60)
    base_tester.METHOD_RECORDED[0]["flag"] = False

    login_response = base_tester.api_request(
        "POST",
        "/auth/auth/apploginMultipleRole",
        need_auth=False,
        data=login_payload,
        timeout=api_config.API_CONFIG["login_timeout"]
    )
    ...
    if login_response.get("success") and login_response.get("data", {}).get("token"):
        ...
        try:
            ...
            print(f"⚠️ JWT解码失败: {e}")
            USER_ID = None
        base_tester.LOGIN_RESPONSE = login_response
        print(f"✅ Token获取成功")
        ...
        for method_name in dir(instance):
            if method_name.startswith("test_"):
                ...
                try:
                    ...
                    if not base_tester.METHOD_RECORDED[0]["flag"]:
                        _elapsed = int((time.time() - _start) * 1000)
                        api_path = getattr(e, 'api_path', '') or base_tester.CURRENT_API_PATH or ""
    ...
    if args.output_json:
        ...
        with open(json_path, "w", encoding="utf-8") as _f:
            json.dump(base_tester.TEST_RESULTS, _f, ensure_ascii=False, indent=2)
        print(f"📄 JSON已保存: {json_path}")
        sys.exit(0)

    base_tester.generate_report(
        base_tester.TEST_RESULTS,
