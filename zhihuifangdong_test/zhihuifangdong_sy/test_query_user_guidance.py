...
# -*- coding: utf-8 -*-
"""
智慧房东 APP - /core/app/operGuide/queryUserGuidance 接口测试
"""
...
class TestUserInfo:
    """/core/app/operGuide/queryUserGuidance"""

    def test_10_query_user_guidance(self):
        """用户引导状态查询"""
        print("\n📋 测试用户引导状态查询...")
        response = base_tester.api_request("POST", "/core/app/operGuide/queryUserGuidance",
                               data={"name": "1"})
        print(f"响应: {base_tester.json.dumps(response, ensure_ascii=False, indent=2)}")
        base_tester.validate_response(response)
        print("✅ 用户引导状态查询完成")
        base_tester.record_result("test_10_query_user_guidance", "/core/app/operGuide/queryUserGuidance", response, "PASSED", group=TEST_GROUP)
...
if __name__ == "__main__":
    ...
    method = getattr(instance, "test_10_query_user_guidance")
    METHOD_RECORDED = False
    try:
        method()
        print("\n✅ " + "test_10_query_user_guidance" + " 执行通过")
        ...
        base_tester.record_result(
            "test_10_query_user_guidance",
            api_path_val,
            exc_response if exc_response else {"success": False, "message": str(e)},
            "FAILED"
        , group=TEST_GROUP)
        print("\n❌ " + "test_10_query_user_guidance" + " 执行失败: " + str(e))
