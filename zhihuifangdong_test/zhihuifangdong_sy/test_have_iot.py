...
# -*- coding: utf-8 -*-
"""
智慧房东 APP - /core/app/user/haveIot 接口测试
"""
...
class TestUserInfo:
    """/core/app/user/haveIot"""

    def test_07_have_iot(self):
        """IoT设备检查"""
        print("\n📋 测试IoT设备检查...")
        response = base_tester.api_request("GET", "/core/app/user/haveIot")
        print(f"响应: {base_tester.json.dumps(response, ensure_ascii=False, indent=2)}")
        base_tester.validate_response(response)
        print("✅ IoT设备检查完成")
        base_tester.record_result("test_07_have_iot", "/core/app/user/haveIot", response, "PASSED", group=TEST_GROUP)
...
if __name__ == "__main__":
    ...
    method = getattr(instance, "test_07_have_iot")
    METHOD_RECORDED = False
    try:
        method()
        print("\n✅ " + "test_07_have_iot" + " 执行通过")
