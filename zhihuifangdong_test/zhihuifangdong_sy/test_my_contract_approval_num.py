...
# -*- coding: utf-8 -*-
"""
智慧房东 APP - /core/app/contractExt/myContractApprovalNum 接口测试
"""
...
class TestContract:
    """/core/app/contractExt/myContractApprovalNum"""

    def test_16_my_contract_approval_num(self):
        """待审批合同数量"""
        print("\n📑 测试待审批合同数量...")
        response = base_tester.api_request("GET", "/core/app/contractExt/myContractApprovalNum")
        print(f"响应: {base_tester.json.dumps(response, ensure_ascii=False, indent=2)}")
        base_tester.validate_response(response)
        print("✅ 待审批合同数量获取成功")
        base_tester.record_result("test_16_my_contract_approval_num", "/core/app/contractExt/myContractApprovalNum", response, "PASSED", group=TEST_GROUP)
...
if __name__ == "__main__":
    ...
    method = getattr(instance, "test_16_my_contract_approval_num")
    METHOD_RECORDED = False
    try:
        method()
        print("\n✅ " + "test_16_my_contract_approval_num" + " 执行通过")
        ...
        base_tester.record_result(
            "test_16_my_contract_approval_num",
            api_path_val,
            exc_response if exc_response else {"success": False, "message": str(e)},
            "FAILED"
        , group=TEST_GROUP)
        print("\n❌ " + "test_16_my_contract_approval_num" + " 执行失败: " + str(e))
