...
class TestActivity:
    ...
    def test_21_platform_pic(self):
        """平台图片"""
        print("\n🎨 测试平台图片...")
        response = base_tester.api_request("GET", "/core/app/activity/platformPic")
        print(f"响应: {base_tester.json.dumps(response, ensure_ascii=False, indent=2)}")
        base_tester.validate_response(response)
        print("✅ 平台图片获取成功")
        base_tester.record_result("test_21_platform_pic", "/core/app/activity/platformPic", response, "PASSED", group=TEST_GROUP)
