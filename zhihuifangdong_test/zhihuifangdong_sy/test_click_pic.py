...
# -*- coding: utf-8 -*-
"""
智慧房东 APP - /core/app/activity/clickPic 接口测试
"""
...
class TestActivity:
    """/core/app/activity/clickPic"""

    def test_19_click_pic(self):
        """点击图片配置"""
        print("\n🎨 测试点击图片配置...")
        response = base_tester.api_request("GET", "/core/app/activity/clickPic")
        print(f"响应: {base_tester.json.dumps(response, ensure_ascii=False, indent=2)}")
        base_tester.validate_response(response)
        print("✅ 点击图片配置获取成功")
        base_tester.record_result("test_19_click_pic", "/core/app/activity/clickPic", response, "PASSED", group=TEST_GROUP)
...
if __name__ == "__main__":
    ...
    method = getattr(instance, "test_19_click_pic")
    METHOD_RECORDED = False
    try:
        method()
        print("\n✅ " + "test_19_click_pic" + " 执行通过")
