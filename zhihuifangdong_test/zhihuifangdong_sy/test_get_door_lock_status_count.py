...
# -*- coding: utf-8 -*-
"""
智慧房东 APP - /netty/app/doorlock/getDoorLockStatusCount 接口测试
"""
...
class TestDevice:
    """/netty/app/doorlock/getDoorLockStatusCount"""

    def test_23_get_door_lock_status_count(self):
        """门锁状态统计"""
        print("\n🔒 测试门锁状态统计...")
        response = base_tester.api_request("GET", "/netty/app/doorlock/getDoorLockStatusCount")
        print(f"响应: {base_tester.json.dumps(response, ensure_ascii=False, indent=2)}")
        base_tester.validate_response(response)
        print("✅ 门锁状态统计获取成功")
        base_tester.record_result("test_23_get_door_lock_status_count", "/netty/app/doorlock/getDoorLockStatusCount", response, "PASSED", group=TEST_GROUP)
...
if __name__ == "__main__":
    ...
    instance = TestDevice()
    method = getattr(instance, "test_23_get_door_lock_status_count")
    METHOD_RECORDED = False
    try:
        method()
        print("\n✅ " + "test_23_get_door_lock_status_count" + " 执行通过")
        ...
        base_tester.record_result(
            "test_23_get_door_lock_status_count",
            api_path_val,
            exc_response if exc_response else {"success": False, "message": str(e)},
            "FAILED"
        , group=TEST_GROUP)
        print("\n❌ " + "test_23_get_door_lock_status_count" + " 执行失败: " + str(e))
