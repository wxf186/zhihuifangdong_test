...
# -*- coding: utf-8 -*-
"""
智慧房东 APP - /core/app/house/lockIndexList 接口测试
"""
...
class TestDevice:
    """/core/app/house/lockIndexList"""

    def test_24_get_door_lock_list(self):
        """门锁设备列表"""
        print("\n🔒 测试门锁设备列表...")
        response = base_tester.api_request("GET", "/core/app/house/lockIndexList",
                               params={"current": 1, "size": 10})
        print(f"响应: {base_tester.json.dumps(response, ensure_ascii=False, indent=2)}")
        base_tester.validate_response(response)
        print("✅ 门锁设备列表查询完成")
        base_tester.record_result("test_24_get_door_lock_list", "/core/app/house/lockIndexList", response, "PASSED", group=TEST_GROUP)
...
if __name__ == "__main__":
    ...
    instance = TestDevice()
    method = getattr(instance, "test_24_get_door_lock_list")
    METHOD_RECORDED = False
    try:
        method()
        print("\n✅ " + "test_24_get_door_lock_list" + " 执行通过")
        ...
        base_tester.record_result(
            "test_24_get_door_lock_list",
            api_path_val,
            exc_response if exc_response else {"success": False, "message": str(e)},
            "FAILED"
        , group=TEST_GROUP)
        print("\n❌ " + "test_24_get_door_lock_list" + " 执行失败: " + str(e))
