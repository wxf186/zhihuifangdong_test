...
# -*- coding: utf-8 -*-
"""
智慧房东 APP - /core/app/user/selfMenuForNew 接口测试
"""
...
class TestUserInfo:
    """/core/app/user/selfMenuForNew"""

    def test_05_self_menu_for_new(self):
        """新版菜单"""
        print("\n📋 测试新版菜单...")
        response = base_tester.api_request("GET", "/core/app/user/selfMenuForNew")
        print(f"响应: {base_tester.json.dumps(response, ensure_ascii=False, indent=2)}")
        base_tester.validate_response(response)
        print("✅ 新版菜单获取成功")
        base_tester.record_result("test_05_self_menu_for_new", "/core/app/user/selfMenuForNew", response, "PASSED", group=TEST_GROUP)
...
if __name__ == "__main__":
    ...
    method = getattr(instance, "test_05_self_menu_for_new")
    METHOD_RECORDED = False
    try:
        method()
        print("\n✅ " + "test_05_self_menu_for_new" + " 执行通过")
        ...
        base_tester.record_result(
            "test_05_self_menu_for_new",
            api_path_val,
            exc_response if exc_response else {"success": False, "message": str(e)},
            "FAILED"
        , group=TEST_GROUP)
        print("\n❌ " + "test_05_self_menu_for_new" + " 执行失败: " + str(e))
