...
# -*- coding: utf-8 -*-
"""
智慧房东 APP - /netty/web/meter/fluxRemindWindow 接口测试
"""
...
class TestMeter:
    ...
    def test_25_flux_remind_window(self):
        """流量提醒弹窗"""
        print("\n📡 测试流量提醒弹窗...")
        _start = time.time()
        time.sleep(0.5)
        response = base_tester.api_request("GET", "/netty/web/meter/fluxRemindWindow",
                               params={"userId": base_tester.USER_ID},
                               timeout=60)
        print(f"响应: {base_tester.json.dumps(response, ensure_ascii=False, indent=2)}")
        try:
            base_tester.validate_response(response)
            base_tester.record_result("test_25_flux_remind_window", "/netty/web/meter/fluxRemindWindow", response, "PASSED", group=TEST_GROUP)
        except AssertionError:
            base_tester.record_result("test_25_flux_remind_window", "/netty/web/meter/fluxRemindWindow", response, "FAILED", group=TEST_GROUP)
            raise
        finally:
            if base_tester.TEST_RESULTS:
                base_tester.TEST_RESULTS[-1]["duration"] = int((time.time() - _start) * 1000)
        print("✅ 流量提醒弹窗查询完成")
...
if __name__ == "__main__":
    ...
    instance = TestMeter()
    method = getattr(instance, "test_25_flux_remind_window")
    base_tester.METHOD_RECORDED[0]["flag"] = False
    try:
        method()
        print("\n✅ " + "test_25_flux_remind_window" + " 执行通过")
    except Exception as e:
        print("\n❌ " + "test_25_flux_remind_window" + " 执行失败: " + str(e))
