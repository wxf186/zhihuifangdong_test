...
# -*- coding: utf-8 -*-
"""
智慧房东 APP - /core/app/bindCard/zdBankMigrateAdmin 接口测试
"""
...
class TestBindCard:
    """/core/app/bindCard/zdBankMigrateAdmin"""

    def test_28_zd_bank_migrate_admin(self):
        """银行迁移状态"""
        print("\n💳 测试银行迁移状态...")
        response = base_tester.api_request("GET", "/core/app/bindCard/zdBankMigrateAdmin")
        print(f"响应: {base_tester.json.dumps(response, ensure_ascii=False, indent=2)}")
        base_tester.validate_response(response)
        print("✅ 银行迁移状态查询完成")
        base_tester.record_result("test_28_zd_bank_migrate_admin", "/core/app/bindCard/zdBankMigrateAdmin", response, "PASSED", group=TEST_GROUP)
...
if __name__ == "__main__":
    ...
    method = getattr(instance, "test_28_zd_bank_migrate_admin")
    METHOD_RECORDED = False
    try:
        method()
        print("\n✅ " + "test_28_zd_bank_migrate_admin" + " 执行通过")
        ...
        base_tester.record_result(
            "test_28_zd_bank_migrate_admin",
            api_path_val,
            exc_response if exc_response else {"success": False, "message": str(e)},
            "FAILED"
        , group=TEST_GROUP)
        print("\n❌ " + "test_28_zd_bank_migrate_admin" + " 执行失败: " + str(e))
