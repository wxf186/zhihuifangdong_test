...
# -*- coding: utf-8 -*-
"""
智慧房东 APP - /core/app/bindCard/getBindCardFailPop 接口测试
"""
...
class TestBindCard:
    """/core/app/bindCard/getBindCardFailPop"""

    def test_27_get_bind_card_fail_pop(self):
        """绑卡失败弹窗（success=false属于正常）"""
        print("\n💳 测试绑卡失败弹窗...")
        response = base_tester.api_request("GET", "/core/app/bindCard/getBindCardFailPop")
        ...
        print("✅ 绑卡失败弹窗查询完成")
        base_tester.record_result("test_27_get_bind_card_fail_pop", "/core/app/bindCard/getBindCardFailPop", response, "PASSED", group=TEST_GROUP)
...
if __name__ == "__main__":
    ...
    instance = TestBindCard()
    method = getattr(instance, "test_27_get_bind_card_fail_pop")
    METHOD_RECORDED = False
    try:
        method()
        print("\n✅ " + "test_27_get_bind_card_fail_pop" + " 执行通过")
        ...
        base_tester.record_result(
            "test_27_get_bind_card_fail_pop",
            api_path_val,
            exc_response if exc_response else {"success": False, "message": str(e)},
            "FAILED"
        , group=TEST_GROUP)
        print("\n❌ " + "test_27_get_bind_card_fail_pop" + " 执行失败: " + str(e))
