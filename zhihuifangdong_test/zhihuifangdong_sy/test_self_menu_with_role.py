...
# -*- coding: utf-8 -*-
"""
智慧房东 APP - /core/app/user/selfMenuWithRole 接口测试
"""
...
class TestUserInfo:
    """/core/app/user/selfMenuWithRole"""

    def test_03_self_menu_with_role(self):
        """角色菜单"""
        print("\n📋 测试角色菜单...")
        response = base_tester.api_request("GET", "/core/app/user/selfMenuWithRole")
        print(f"响应: {base_tester.json.dumps(response, ensure_ascii=False, indent=2)}")
        base_tester.validate_response(response)
        print("✅ 角色菜单获取成功")
        base_tester.record_result("test_03_self_menu_with_role", "/core/app/user/selfMenuWithRole", response, "PASSED", group=TEST_GROUP)
...
if __name__ == "__main__":
    ...
    method = getattr(instance, "test_03_self_menu_with_role")
    METHOD_RECORDED = False
    try:
        method()
        print("\n✅ " + "test_03_self_menu_with_role" + " 执行通过")
        ...
        base_tester.record_result(
            "test_03_self_menu_with_role",
            api_path_val,
            exc_response if exc_response else {"success": False, "message": str(e)},
            "FAILED"
        , group=TEST_GROUP)
        print("\n❌ " + "test_03_self_menu_with_role" + " 执行失败: " + str(e))
