...
# -*- coding: utf-8 -*-
"""
智慧房东 APP - /core/app/house/waterIndexList 接口测试
"""
...
class TestMeter:
    """/core/app/house/waterIndexList"""

    def test_31_water_meter_list(self):
        """水表设备列表"""
        print("\n💧 测试水表设备列表...")
        response = base_tester.api_request("GET", "/core/app/house/waterIndexList",
                               params={"current": 1, "size": 10})
        print(f"响应: {base_tester.json.dumps(response, ensure_ascii=False, indent=2)}")
        base_tester.validate_response(response)
        print("✅ 水表设备列表查询完成")
        base_tester.record_result("test_31_water_meter_list", "/core/app/house/waterIndexList", response, "PASSED", group=TEST_GROUP)
...
if __name__ == "__main__":
    ...
    instance = TestMeter()
    method = getattr(instance, "test_31_water_meter_list")
    METHOD_RECORDED = False
    try:
        method()
        print("\n✅ " + "test_31_water_meter_list" + " 执行通过")
