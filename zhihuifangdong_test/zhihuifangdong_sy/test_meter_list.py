...
# -*- coding: utf-8 -*-
"""
智慧房东 APP - /core/app/house/meterIndexList 接口测试
"""
...
class TestMeter:
    """/core/app/house/meterIndexList"""

    def test_32_meter_list(self):
        """电表设备列表"""
        print("\n⚡ 测试电表设备列表...")
        response = base_tester.api_request("GET", "/core/app/house/meterIndexList",
                               params={"current": 1, "size": 10})
        print(f"响应: {base_tester.json.dumps(response, ensure_ascii=False, indent=2)}")
        base_tester.validate_response(response)
        print("✅ 电表设备列表查询完成")
        base_tester.record_result("test_32_meter_list", "/core/app/house/meterIndexList", response, "PASSED", group=TEST_GROUP)
...
if __name__ == "__main__":
    ...
    instance = TestMeter()
    method = getattr(instance, "test_32_meter_list")
    METHOD_RECORDED = False
    try:
        method()
        print("\n✅ " + "test_32_meter_list" + " 执行通过")
