...
# -*- coding: utf-8 -*-
"""
智慧房东 APP - /core/contractApproval/list 接口测试
"""
...
class TestContract:
    """/core/contractApproval/list"""

    def test_17_contract_approval_list(self):
        """合同审批列表"""
        print("\n📑 测试合同审批列表...")
        response = base_tester.api_request("POST", "/core/contractApproval/list",
                               json_data={"approvalStatus": "INIT", "current": 1, "size": 9999})
        print(f"响应: {base_tester.json.dumps(response, ensure_ascii=False, indent=2)}")
        base_tester.validate_response(response)
        print("✅ 合同审批列表获取成功")
        base_tester.record_result("test_17_contract_approval_list", "/core/contractApproval/list", response, "PASSED", group=TEST_GROUP)
...
if __name__ == "__main__":
    ...
    method = getattr(instance, "test_17_contract_approval_list")
    METHOD_RECORDED = False
    try:
        method()
        print("\n✅ " + "test_17_contract_approval_list" + " 执行通过")
