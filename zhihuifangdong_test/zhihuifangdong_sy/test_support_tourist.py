...
# -*- coding: utf-8 -*-
"""
智慧房东 APP - /findhouse/app/houseScore/supportTourist 接口测试
"""
...
class TestHousePromotion:
    """/findhouse/app/houseScore/supportTourist"""

    def test_29_support_tourist(self):
        """房源推广支持"""
        print("\n🏠 测试房源推广支持...")
        response = base_tester.api_request("GET", "/findhouse/app/houseScore/supportTourist")
        print(f"响应: {base_tester.json.dumps(response, ensure_ascii=False, indent=2)}")
        base_tester.validate_response(response)
        print("✅ 房源推广支持查询完成")
        base_tester.record_result("test_29_support_tourist", "/findhouse/app/houseScore/supportTourist", response, "PASSED", group=TEST_GROUP)
...
if __name__ == "__main__":
    ...
    instance = TestHousePromotion()
    method = getattr(instance, "test_29_support_tourist")
    METHOD_RECORDED = False
    try:
        method()
        print("\n✅ " + "test_29_support_tourist" + " 执行通过")
