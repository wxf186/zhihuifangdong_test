...
# -*- coding: utf-8 -*-
"""
智慧房东 APP - /core/feign/userExtend/addMecAddress 接口测试
"""
...
class TestFeign:
    """/core/feign/userExtend/addMecAddress"""

    def test_30_add_mec_address(self):
        """MAC地址上报"""
        print("\n📡 测试MAC地址上报...")
        time.sleep(0.3)
        response = base_tester.api_request("POST", "/core/feign/userExtend/addMecAddress",
                               data={"macAddress": "BFE2048DB90664D3EBF6CB14306202BA"})
        print(f"响应: {base_tester.json.dumps(response, ensure_ascii=False, indent=2)}")
        base_tester.validate_response(response)
        print("✅ MAC地址上报完成")
        base_tester.record_result("test_30_add_mec_address", "/core/feign/userExtend/addMecAddress", response, "PASSED", group=TEST_GROUP)
...
if __name__ == "__main__":
    ...
    instance = TestFeign()
    method = getattr(instance, "test_30_add_mec_address")
    METHOD_RECORDED = False
    try:
        method()
        print("\n✅ " + "test_30_add_mec_address" + " 执行通过")
