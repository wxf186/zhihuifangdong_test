...
# -*- coding: utf-8 -*-
"""
智慧房东 APP - /core/app/user/info 接口测试
"""
...
class TestUserInfo:
    ...
    def test_02_user_info(self):
        """获取用户信息"""
        print("\n📋 测试获取用户信息...")
        response = base_tester.api_request("GET", "/core/app/user/info")
        ...
        assert response["data"]["username"] == USERS[api_config.ACTIVE_ENV]["username"]
        print(f"✅ 用户信息验证通过")
        base_tester.record_result("test_02_user_info", "/core/app/user/info", response, "PASSED", group=TEST_GROUP)
...
if __name__ == "__main__":
    ...
    method = getattr(instance, "test_02_user_info")
    METHOD_RECORDED = False
    try:
        method()
        print("\n✅ " + "test_02_user_info" + " 执行通过")
