...
# -*- coding: utf-8 -*-
"""
智慧房东 APP - /core/checkHouseIndexPlus 接口测试
"""
...
class TestHouseIndex:
    """/core/checkHouseIndexPlus"""

    def test_10_check_house_index_plus(self):
        """房源完整性校验（合租）"""
        print("\n📋 测试房源完整性校验（合租）...")
        ...
        base_tester.record_result("test_10_check_house_index_plus", "/core/checkHouseIndexPlus", response, status, group=TEST_GROUP)
        if not response.get("success"):
            e = AssertionError(f"业务失败: {response.get('message', '未知错误')}")
            e.response = response
            e.api_path = "/core/checkHouseIndexPlus"
            raise e
        print(f"✅ 房源完整性校验成功")
...
if __name__ == "__main__":
    ...
    method = getattr(instance, "test_10_check_house_index_plus")
    METHOD_RECORDED = False
    try:
        method()
        print("\n✅ " + "test_10_check_house_index_plus" + " 执行通过")
