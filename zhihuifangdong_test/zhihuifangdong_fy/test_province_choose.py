...
# -*- coding: utf-8 -*-
"""
智慧房东 APP - /core/app/house/provinceChoose 接口测试
"""
...
TEST_GROUP = "fy"
class TestDistrict:
    """/core/app/house/provinceChoose"""

    def test_12_province_choose(self):
        """省份列表获取"""
        print("\n📋 测试省份列表获取...")
        response = base_tester.api_request("GET", "/core/app/house/provinceChoose")
        print(f"响应: {base_tester.json.dumps(response, ensure_ascii=False, indent=2)[:300]}")
        base_tester.validate_response(response)
        print(f"✅ 省份列表获取成功")
        base_tester.record_result("test_12_province_choose", "/core/app/house/provinceChoose", response, "PASSED", group=TEST_GROUP)
...
if __name__ == "__main__":
    ...
    instance = TestDistrict()
    method = getattr(instance, "test_12_province_choose")
    METHOD_RECORDED = False
    try:
        method()
        print("\n✅ " + "test_12_province_choose" + " 执行通过")
