...
# -*- coding: utf-8 -*-
"""
智慧房东 APP - /core/wholeHouse/getBlock 接口测试
"""
...
TEST_GROUP = "fy"
class TestCotenancy:
    """/core/wholeHouse/getBlock"""

    def test_04_get_block(self):
        """合租栋座获取"""
        print("\n📋 测试合租栋座获取...")
        response = base_tester.api_request(
            "GET",
            "/core/wholeHouse/getBlock",
            params={"type": "COTENANCY"}
        )
        print(f"响应: {base_tester.json.dumps(response, ensure_ascii=False, indent=2)[:300]}")
        base_tester.validate_response(response)
        print(f"✅ 合租栋座获取成功")
        base_tester.record_result("test_04_get_block", "/core/wholeHouse/getBlock", response, "PASSED", group=TEST_GROUP)
...
if __name__ == "__main__":
    ...
    method = getattr(instance, "test_04_get_block")
    METHOD_RECORDED = False
    try:
        method()
        print("\n✅ " + "test_04_get_block" + " 执行通过")
