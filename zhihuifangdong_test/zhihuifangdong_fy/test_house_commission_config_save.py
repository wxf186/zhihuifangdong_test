...
class TestCommission:
    ...
    def test_11_house_commission_config_save(self):
        """佣金配置保存"""
        print("\n📋 测试佣金配置保存...")
        # 文档: POST /findhouse/houseCommissionCofig/save（注意Cofig拼写）
        # Body: houseId + houseRent + configs（form-urlencoded，configs为JSON数组字符串）
        configs_json = base_tester.json.dumps([
            {"commissionType": 2, "commissionValue": 50, "startMonth": 1, "endMonth": None}
        ])
        ...
        print(f"✅ 佣金配置保存成功")
        base_tester.record_result("test_11_house_commission_config_save", "/findhouse/houseCommissionCofig/save", response, "PASSED", group=TEST_GROUP)
