...
# -*- coding: utf-8 -*-
"""
智慧房东 APP - /core/app/houseManageExt/accurateSearch 接口测试
"""
...
TEST_GROUP = "fy"
class TestSearch:
    """/core/app/houseManageExt/accurateSearch"""

    def test_02_accurate_search(self):
        """小区精确搜索"""
        print("\n📋 测试小区精确搜索...")
        response = base_tester.api_request(
            "POST",
            "/core/app/houseManageExt/accurateSearch",
            data={"rentType": "WHOLE", "keyWord": "东"}
        )
        print(f"响应: {base_tester.json.dumps(response, ensure_ascii=False, indent=2)[:300]}")
        base_tester.validate_response(response)
        assert response["data"].get("communityList") is not None
        print(f"✅ 小区精确搜索成功")
        base_tester.record_result("test_02_accurate_search", "/core/app/houseManageExt/accurateSearch", response, "PASSED", group=TEST_GROUP)
...
if __name__ == "__main__":
    ...
    instance = TestSearch()
    method = getattr(instance, "test_02_accurate_search")
    METHOD_RECORDED = False
    try:
        method()
        print("\n✅ " + "test_02_accurate_search" + " 执行通过")
